    		...
    		<tr id='tr_parent_0'>
                <?php
        			...
        			if ( isset( $_GET[ 'TariffId' ] ) ) {
        				$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( AccountServices.TariffId = " . $_GET[ 'TariffId' ] . " )";
        			}
        			
            		
            		if ( isset( $_GET[ 'ServiceId' ] ) ) {
            			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( AccountServices.ServiceId = " . $_GET[ 'ServiceId' ] . " )";
            		}
            		
            		if ( isset( $_GET[ 'AccountId' ] ) ) {
            			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( AccountServices.AccountId = " . $_GET[ 'AccountId' ] . " )";
            		}
		...
		<?php
			if ( isset( $data ) ) {
				...
				foreach ( $data as $value ) {
					...
					echo "<td width='20px'><a href='#ex3' onclick=\"DeleteAccountServices('AccountServices', " . $value[ 'AccountId' ] . ",".$value[ 'ServiceId' ].",".$value[ 'TariffId' ].",'".$value[ 'Date' ]."')\" rel='modal:open'><img src='../images/Delete.png' alt='Удалить запись'></a></td>";
			...
			require_once '../includes/modalAccountServices.php';
		?>
	</table>
</div>
<script>
    
    function DeleteAccountServices(table, elem, elem2, elem3, elem4) {
        $('#servAccountId').val(elem);
