...
<table id="data_table">
	<?php
		...
		if ( isset( $_GET[ 'TariffId' ] ) ) {
			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( Tariffs.TariffId = " . $_GET[ 'TariffId' ] . " )";
		}
	
		if ( isset( $_GET[ 'RegionId' ] ) ) {
			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( Tariffs.RegionId = " . $_GET[ 'RegionId' ] . " )";
		}
        
        if ( isset( $_GET[ 'ServiceId' ] ) ) {
			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( Tariffs.ServiceId = " . $_GET[ 'ServiceId' ] . " )";
		}
		
		if ( isset( $_GET[ 'CompanyId' ] ) ) {
			$where = ( empty( $where ) ? "WHERE " : " AND " ) . "( Tariffs.CompanyId = " . $_GET[ 'CompanyId' ] . " )";
		}
		...
		<?php
			if ( isset( $data ) ) {
				...
				foreach ( $data as $value ) {
				    ...
				    echo "<td width='20px'><a href='#ex3' onclick=\"DeleteTariffs('Tariffs', " . $value['TariffId'] . "," . $value['ServiceId']. "," . $value['RegionId'] . ")\" rel='modal:open'><img src='../images/Delete.png' alt='Удалить запись'></a></td>";
				    $ct = $ct + 1;
				    echo "</tr>";
				}
			}
			require_once '../includes/modalTariffs.php';
		?>
	</table>
</div>
...
<script>
	...
	function DeleteTariffs(table, elem, elem2, elem3) {
		$('#popup_id').val(elem);
