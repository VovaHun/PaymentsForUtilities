...
<body>
	<?php
		...
		/*Массив значений при редактировании */
		//$arrayZn = $_POST["fn"];
		/*print_r($arrayZn);*/
		...
		<div class="container content">
			<div class="main__left">
					...
					<div class="container">
						...
						<form class="" method="post" action="/GlobalAdmins/create_update_query/update_query_abonents.php">
							...
							<table>
								<tbody>
									...
									<tr> <td><label>Фактический адрес или адрес проживания:</label></td>
										...
										<?php 
											if($res==-1){
												echo "<input type=text pattern='^[ 0-9]+$' name=inn   maxlength=12 value = > ";	
											}
											else{
												echo "<input type=text pattern='^[ 0-9]+$' name=inn maxlength=12 value = '".$row['INN']."'> ";
											}
											?>
										</td>								
									</tr>
									...
									<tr> <td><label>КПП (для юр.лиц):</label></td>
										...
										<?php 
											if($res==-1){
												echo "<input type=text pattern='^[ 0-9]+$' name=kpp maxlength=9 value = > ";	
											}
											else{
												echo "<input type=text pattern='^[ 0-9]+$' name=kpp maxlength=9 value = '".$row['KPP']."'> ";
											}
											?>
										</td>								
									</tr>
									...
									<tr> <td><label>Расчетный счет (юр.лица):</label></td>
										...
										<?php 
											if($res==-1){
												echo "<input type=number name=payAc value = > ";	
											}
											else{
												echo "<input type=number name=payAc value = '".$row['PaymentAccount']."'> ";
											}
											?>
										</td>								
									</tr>
									...
									<tr> <td><label>БИК (юр.лица):</label></td>
										...
										<?php 
											if($res==-1){
												echo "<input type=text pattern='^[ 0-9]+$' name=bik maxlength=9 value = > ";	
											}
											else{
												echo "<input type=text pattern='^[ 0-9]+$' name=bik maxlength=9 value = '".$row['BIK']."'> ";
											}
											?>
										</td>								
									</tr>
							...
							<div>
								...
								<tr>
									<td>
										...
										<?php 
            								echo "<a href='/GlobalAdmins/?table=Abonents'>Отменить</a>";
            							?>
									</td>
								</tr>
								<?php
