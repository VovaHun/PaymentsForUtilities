    		...
    		<tr id='tr_parent_0'>
                <?php
        			...
        			if ( isset( $_GET[ 'TariffId' ] ) ) {
        				$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountNormatives.TariffId = " . $_GET[ 'TariffId' ] . " )";
        			}
            		
            		if ( isset( $_GET[ 'ServiceId' ] ) ) {
            			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountNormatives.ServiceId = " . $_GET[ 'ServiceId' ] . " )";
            		}
            		
            		if ( isset( $_GET[ 'AccountId' ] ) ) {
            			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountNormatives.AccountId = " . $_GET[ 'AccountId' ] . " )";
            		}
