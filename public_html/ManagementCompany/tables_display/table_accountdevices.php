    		...
    		<tr id='tr_parent_0'>
                <?php
        			...
        			if ( isset( $_GET[ 'TariffId' ] ) ) {
        				$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountDevices.TariffId = " . $_GET[ 'TariffId' ] . " )";
        			}
        			
        			if ( isset( $_GET[ 'DeviceId' ] ) ) {
            			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountDevices.DeviceId = " . $_GET[ 'DeviceId' ] . " )";
            		}
            		
            		if ( isset( $_GET[ 'ServiceId' ] ) ) {
            			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountDevices.ServiceId = " . $_GET[ 'ServiceId' ] . " )";
            		}
            		
            		if ( isset( $_GET[ '' ] ) ) {
            			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( AccountDevices.AccountId = " . $_GET[ 'AccountId' ] . " )";
            		}
