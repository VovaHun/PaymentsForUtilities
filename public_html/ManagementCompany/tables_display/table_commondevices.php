    ...
    <table id="data_table">
    	...
    	<?php
    		...
    		if ( isset( $_GET[ 'TariffId' ] ) ) {
    			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( CommonDevices.TariffId = " . $_GET[ 'TariffId' ] . " )";
    		}
    
    		if ( isset( $_GET[ 'DeviceId' ] ) ) {
    			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( CommonDevices.DeviceId = " . $_GET[ 'DeviceId' ] . " )";
    		}
    		
        	    
            if ( isset( $_GET[ 'ServiceId' ] ) ) {
    			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( CommonDevices.ServiceId = " . $_GET[ 'ServiceId' ] . " )";
    		}
    		
    		if ( isset( $_GET[ 'ObjectId' ] ) ) {
    			$where =$where.( empty( $where ) ? "WHERE " : " AND " ) . "( CommonDevices.ObjectId = " . $_GET[ 'ObjectId' ] . " )";
    		}
    		...
    		$query1 ="SELECT 
                        ...
                        Objects.CompanyId AS ObCompanyId,
